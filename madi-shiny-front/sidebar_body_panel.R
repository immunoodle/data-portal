...
observeEvent(input$rb, {
    ...
    if(input$rb == "exploreData"){
      ...
      insertTab(inputId = "body_panel_id",
                tabPanel(value = "study"
                         ...
                         , shinydashboard::box(title = "All Studies: Data Table"
  ...
  if(input$rb == "immportUpload"){
    ...
    if(input$rb == "adminTasks"){
      ...
      removeTab(inputId = "body_panel_id", target="showAssociated")
      removeTab(inputId = "body_panel_id", target="add_workspace_tab")
      updateRadioButtons(session, "rb_admin",
                         choiceNames = list(
                           HTML("<p style='color:#2c3e50; font-weight: 500;'>Add users</p>")
                           ,HTML("<p style='color:#2c3e50; font-weight: 500;'>Add new program</p>")
                           ,HTML("<p style='color:#2c3e50; font-weight: 500;'>Add new personnel</p>")
                           ,HTML("<p style='color:#2c3e50; font-weight: 500;'>Add program 2 personnel</p>")
                           ,HTML("<p style='color:#2c3e50; font-weight: 500;'>Add workspace</p>")
                         ),
                         choiceValues = list(
                           "add_users", "add_program", "add_personnel", "add_program_2_personnel", "add_workspace"
                         ), selected = character(0)
                      )
...
observeEvent(input$rb_admin, {
  if(input$rb_add == "add_users"){
    removeTab(inputId = "body_panel_id", target="add_workspace_tab")
    removeTab(inputId = "body_panel_id", target = "study_personnel")
    removeTab(inputId = "body_panel_id", target = "program_2_personnel")
    removeTab(inputId = "body_panel_id", target = "add_personnel_tab")
  ...
  if(input$rb_add == "add_program_2_personnel"){
      removeTab(inputId = "body_panel_id", target="add_workspace_tab")
      removeTab(inputId = "body_panel_id", target = "add_users_tab")
      removeTab(inputId = "body_panel_id", target = "study_personnel")
      removeTab(inputId = "body_panel_id", target = "add_personnel_tab")
  ...
  updateTabsetPanel(session, "body_panel_id", selected = input$rb_admin)
  })
